...
// LAIG PROB1_inicio
            case 'rectangle2':                    
                ...
                // y2
                var offset = this.reader.getFloat(grandChildren[0], 'corneroffset');
                if (!(offset != null && !isNaN(offset) && offset >= 0 && offset <= 100))
                    return "unable to parse corneroffset of the primitive coordinates with ID = " + primitiveID;

                var rect2 = new MyRectangle2(this.scene, x1, x2, y1, y2, offset);
                this.primitives[primitiveID] = rect2;
                break;
// LAIG PROB1_fim
    ...
    parseComponents(componentsNode) {
        ...
        for (var i = 0; i < children.length; i++) {
            ...
            var matmarkIndex = nodeNames.indexOf("matmark");
...
// LAIG PROB2_inicio  
            ...
            var matmarkId = null;
            if(matmarkId != -1) {
                matmarkId = this.reader.getString(grandChildren[matmarkIndex], "id");
                if (matmarkId != null && !this.materials[matmarkId]){
                    this.onXMLMinorError("material not defined for matmark with ID " + matmarkId + " in component " + componentID + " assuming no animation");
                    matmarkId = null;
                }
            }
...
// LAIG PROB2_inicio  
            ...
            if(animationIndex != -1) {
                ...
                if (animationId != null && !this.animations[animationId]){
                    this.onXMLMinorError("animation not defined for animation with ID " + animationId + " in component " + componentID + " assuming no animation");
                    animationId = null;
                }
            }
...
// LAIG PROB2_inicio  
        //get material marked
        if(this.scene.materialMarkVisible && node.matmark != null) {
            let matIndex = this.materials.indexOf(node.matmark);
            material = node.materials[matIndex];
        }
